...
export default async function TaskDetailPage({
  ...
  const dependencyOptions = dashboard.tasks
    .filter((item) => item.id !== task.id && item.status !== "ARCHIVED")
    .map((item) => ({
      id: item.id,
      name: item.name
    }));
  ...
  return (
    ...
    >
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="panel planner-paper space-y-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-[1.4rem] border border-ink/10 bg-white/90 p-4">
              <p className="text-sm text-ink/55">Depends on</p>
              <p className="mt-1 font-semibold">{task.dependsOnTask?.name ?? "None"}</p>
            </div>
            <div className="rounded-[1.4rem] border border-ink/10 bg-white/90 p-4">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <TaskForm categories={dashboard.categories} dependencyOptions={dependencyOptions} task={task} />
          ...
          <div className="space-y-3">
            {task.continuations.length ? (
              task.continuations.map((continuation) => (
                <Link className="sticker-card block" href={`/planner/tasks/${continuation.id}`} key={continuation.id}>
